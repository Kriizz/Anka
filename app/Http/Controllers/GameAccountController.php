...
class GameAccountController extends Controller
{
    ...
    public function cancel(Request $request,$server,$accountId,$characterId)
    {
        ...
        $resell->delete();
        $world = WorldCharacter::on($server.'_auth')->where('CharacterId',$characterId)->first();
        $world->AccountId = $accountId;
        $world->save();
            $request->session()->flash('notify', ['type' => 'success', 'message' => "Ce personnage à été annuler !"]);
    ...
    public function jetons(Request $request, $server, $accountId)
    {
       ...
       /* if (!$world || !$world->isOnline())
        ...
        if ($request->all())
        {
           ...
           Auth::user()->points = Auth::user()->points + $jetons;
           Auth::user()->save();
           Auth::user()->jetons -= $jetons;
           Auth::user()->save();
           $account = Account::on($server . '_auth')->where('Id', $accountId)->first();
           $account->Tokens = $account->Tokens + $ogrines;
           $account->save();
            Cache::forget('transferts_' . $server . '_' . $accountId);
            Cache::forget('transferts_' . $server . '_' . $accountId . '_10');
            ...
            return redirect()->route('gameaccount.view', ["intel", $account->Id]);
        }
    ...
    public function gifts(Request $request, $server, $accountId)
    {
        ...
        if ($request->all())
        {
            ...
            $accountWorld = Account::on($server.'_world')->where('Id',$accountId)->first();
            if($accountWorld != null)
            {
               
                    $items = new CharacterBank;
                    $items->setConnection('intel_world');
                    $items->AccountId = $accountId;
                    $items->Stack = 1;
                    $items->Position = 63;
                    $items->ItemId = $gift->item_id;
                    $effect = ItemTemplate::on('intel_world')->where('Id',$gift->item_id)->first();
                    $items->SerializedEffects = $effect->PossibleEffectsBin;
                    $items->save();    
            }else
            {
                return redirect()->back();
