...
Route::group(['prefix' => $locale, 'domain' => Config::get('dofus.domain.main')], function() {
    ...
    Route::any('/', [
		...
		Route::get('achat/points/', [
		'middleware' => 'auth',
        'uses' => 'AccountController@buypoints',
        'as'   => 'achatPoints'
    ]);
	Route::post('achat/check/', [
		'middleware' => 'auth',
        'uses' => 'AccountController@buycheck',
        'as'   => 'achatcheck'
    ]);
    /* ============ NEWS ============ */
    ...
    Route::get('sitemap.xml', function() {
        ...
        if (!$sitemap->isCached())
        {
            ...
            $sitemap->add(URL::route('ladder.general', ['hyrule']),   date('c', time()), '0.5', 'daily');
            $sitemap->add(URL::route('ladder.pvp',     ['hyrule']),   date('c', time()), '0.5', 'daily');
            $sitemap->add(URL::route('ladder.guild',   ['hyrule']),   date('c', time()), '0.5', 'daily');
...
Route::group(['prefix' => 'api', 'domain' => Config::get('dofus.domain.main')], function()
{
    ...
    {
        Route::get('create',['uses'=>'SupportController@create','as'=>'support.create']);
